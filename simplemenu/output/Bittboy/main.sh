...
# Background battery monitor: if voltage drops while the launcher runs,
# terminate the launcher, flush state, then power off cleanly.
check_battery() {
	while true; do
		sleep 300
		if test "$(cat /sys/class/power_supply/miyoo-battery/voltage_now)" -lt '3400'; then
...
do
	if test -f "${START}"; then
		...
		( cd "${HOME}"/gmenu2x && ./gmenu2x >> /dev/null 2>&1 ) &
	fi
	launcher_pid="${!}"

	check_battery "${launcher_pid}" &
	monitor_pid="${!}"
